...
mod tests {
    ...
    fn test_fallback_same_province() {
        ...
        assert!((cost - FALLBACK_SAME_PROVINCE).abs() < 0.01);
    ...
    fn test_fallback_adjacent() {
        ...
        assert!((cost - FALLBACK_ADJACENT).abs() < 0.01);
    }
    ...
    fn test_fallback_national() {
        ...
        assert!((cost - NATIONAL_CEILING).abs() < 0.01);
    }
    ...
    fn test_fallback_express_uses_regional_multiplier() {
        ...
        assert!((express / standard - EXPRESS_REGIONAL).abs() < 0.01);
    }
    ...
    fn test_express_multiplier_applied() {
        ...
        assert!((express / standard - EXPRESS_LOCAL).abs() < 0.01);
    }
    ...
    fn test_fallback_same_region_non_adjacent() {
        ...
        assert!((cost - FALLBACK_SAME_REGION).abs() < 0.01);
    }
    ...
    fn test_tiered_additional_item_rate_second_item() {
        ...
        assert!((breakdown["cart_first"] - expected_first).abs() < 0.01);
        assert!((breakdown["cart_second"] - expected_second).abs() < 0.01);
        assert!((total - (expected_first + expected_second)).abs() < 0.01);
    }
    ...
    fn test_same_day_tiered_with_multiplier() {
        ...
        assert!((same_day / standard - SAME_DAY_HYPER_LOCAL).abs() < 0.01);
    }
    ...
    fn test_fallback_second_item_additional_rate() {
        ...
        assert!((breakdown["ci1"] - expected_first).abs() < 0.01);
        assert!((breakdown["ci2"] - expected_second).abs() < 0.01);
        assert!((total - (expected_first + expected_second)).abs() < 0.01);
    }
